...
public class LevelTwo {
    ...
    public LevelTwo(final MyGdxGame game) {
        ...
        movingObstacles = new Rectangle[2];
        ...
        jumpingObstacles[0] = new Rectangle(800, 0, obstacleWidth, 32);
        jumpingObstacles[1] = new Rectangle(1200, 0, obstacleWidth, 32);

        movingObstacles[0] = new Rectangle(400, obstacleY, obstacleWidth, obstacleHeight);
        movingObstacles[1] = new Rectangle(1200, obstacleY, obstacleWidth, obstacleHeight);
    ...
    private void updateVerticalMovingObstacles(float deltaTime) {
        ...
        for (Rectangle obstacle : jumpingObstacles) {
            if (!isObstacleJumping[getObstacleIndex(obstacle)]) {
                ...
                obstacleJumpVelocity[getObstacleIndex(obstacle)] = 1000;
            }
    ...
    private void checkCollisions() {
        ...
        for (Rectangle obstacle : jumpingObstacles) {
            if (obstacle.overlaps(box)) {
                ...
                resetLevel();
                isPaused = false;
                ((MyGdxGame) Gdx.app.getApplicationListener()).showGameOverScreen(2);
                }
    ...
    private void checkLevelPassed() {
        if (box.x + box.width == Gdx.graphics.getWidth()) {
            newLevelSound.play();
            ((MyGdxGame) Gdx.app.getApplicationListener()).showLevelPassedScreen(3);
